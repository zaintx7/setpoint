...
import { Home, PenLine, PlusCircle, CalendarDays, Flame, TrendingUp, Trash2, ChevronLeft, ChevronRight, Minus, Plus, X, Dumbbell, ClipboardList } from "lucide-react";
    ...
    <div className="tab-panel">
      ...
      {workouts.length === 0 ? (
        <div className="empty">
          ...
          <p className="empty-copy">Type your workouts ahead of time. They'll show up in a dropdown at the gym — no typing needed once you're there.</p>
        </div>
      ) : (
...
function LogTab({ workouts, logs, onLog }) {
  ...
  return (
    <div className="tab-panel">
      <select className="workout-select" value={workoutId} onChange={(e) => setWorkoutId(e.target.value)}>
        {workouts.map((w) => (
...
export default function App() {
  ...
  const addWorkout = (name) => {
    const next = [...workouts, { id: uid(), name }];
    setWorkouts(next);
    persist(next, logs);
  };
  ...
  return (
    <div className="sp-app">
      ...
      <div className="tabs">
        ...
        <button className={`tab-btn ${tab === "calendar" ? "tab-btn-active" : ""}`} onClick={() => setTab("calendar")}>
...
function Styles() {
  return (
    <style>{`
      ...
      .stepper-value { font-family: 'JetBrains Mono', monospace; font-weight: 700; font-size: 22px; min-width: 44px; text-align: center; }
